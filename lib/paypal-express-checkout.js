 ...
 */
function Paypal(apiUsername, apiPassword, signature, debug, locale) {
 ...
 * @return {PayPal}                 [description]
 */
Paypal.prototype.setExpressCheckoutPayment = function (email, invoiceNumber, itemAmount, taxAmount, shippingAmount, totalAmount, description, currency, returnUrl, cancelUrl, onlyPayPalUsers, callback) {
  var self = this;
  var params = self.params();
  if (email) {
    params.EMAIL = email;
  ...
  params.PAYMENTREQUEST_0_TAXAMT = prepareNumber(taxAmount);
  params.PAYMENTREQUEST_0_SHIPPINGAMT = prepareNumber(shippingAmount);
  params.PAYMENTREQUEST_0_AMT = prepareNumber(totalAmount);
 ...
 */
exports.create = function (username, password, signature, debug, locale) {
  return new Paypal(username, password, signature, debug, locale);
};
